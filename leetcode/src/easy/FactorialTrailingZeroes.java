 ...
 *   A trailing zero is always produced by prime factors 2 and 5. If we can count the number of 5s and 2s, 
 *   our task is done. Consider the following examples.
 *   n = 5: There is one 5 and 3 2s in prime factors of 5! (2 * 2 * 2 * 3 * 5). 
 *   So count of trailing 0s is 1.
 ...
 *   A simple way is to calculate floor(n/5). For example, 7! has one 5, 10! has two 5s. It is done yet,
 ...
 *   first divide n by 5 and remove all single 5s, then divide by 25 to remove extra 5s and so on.
...
public class FactorialTrailingZeroes {
    ...
    public int trailingZeroes(int n) {
        ...
        for(long i = 5; n >= i; i *= 5) // i必须为long，否则溢出
            count += n / i;
